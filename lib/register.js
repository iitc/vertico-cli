...
var registerEndPoint = VF_CONFIG.register;
var validateAPIKey = VF_CONFIG.validateAPIKey;
var registerInstance = VF_CONFIG.registerInstance;
var instanceEndpoint = VF_CONFIG.instanceEndpoint;
...
exports.register = function (email, secret) {
    if (Util.check()) {
        APP.info("I think you have already registered, why again !");
        return;
    };
    console.log(validateAPIKey + '?email=' + email + '&secretKey=' + secret)
    request.get(validateAPIKey + '?email=' + email + '&secretKey=' + secret)
        ...
        .end(function (err, data) {
            ...
            APP.debug("User registered id = %s",data.body[0].id);
            var donorUserID = data.body[0].id;
...
exports.refreshMyStatus = function (donorId, instanceID) {
    APP.info("Refreshing my status %s",donorId);
    if (!Util.check()) return;
     sys.refresh().then(sysData => {
                APP.silly("Fetched current state of the machine ");
                var instanceObj = {
                    id:instanceID,
                    os: sysData.osInfo.distro,
                    privateIP: sysData.internal_ip,
                ...
                request.post(instanceEndpoint)
                    .set('Content-Type', 'application/json')
                    .send(instanceObj)
                    .end(function (err, success) {
                        if (err) {
                            APP.debug(" Error occured while updating the refresh status %s",instanceEndpoint);
                            APP.error(err);
                            return;
                        }
                        APP.debug("System instance current status is Updated to vertico-factory service!")
                    });
...
exports.unregister = function (email, secret) {
    APP.warn("todo : clean up code is not done yet");
    APP.silly("Un registerting it");
    fs.remove(VF_CONFIG.installedPath, err => {
        if (err) return APP.error("Un registerting it");
        APP.info("Deleted the folder");
    })
