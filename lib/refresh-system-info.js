...
'use strict';
const debug = require('debug')('system-status')  
const name = 'vertico'  
const { promisify } = require('util');
const getIP = promisify(require('external-ip')()); 
const si = require('systeminformation');

 // formatUserFriendlyMemory = true ==> Memory in terms of GB 
var flag = true;
 ...
 exports.refresh = function () {
     ...
     function getSystem() {
         return si.system().then(data => {
             ...
             delete data['uuid'];
             info.system = data;
             debug('Fetched other system info', new Date().getMilliseconds())  
             return info;
         });
     }
     ...
     function getCPU() {
         return si.cpu().then(data => {
             info.cpu = data;
             return info;
             debug('Fetched CPU info %s', new Date()  )
         });
